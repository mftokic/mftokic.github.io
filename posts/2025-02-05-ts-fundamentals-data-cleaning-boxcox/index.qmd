...
We would always prefer to model a time series that has a linear [trend](https://mftokic.github.io/posts/2024-09-25-ts-fundamentals-glossary/#trend). But often that's not the case in businesses. Revenue might double every year, and that my friend is not a linear line, but instead an exponential one. A time series growing exponentially is harder for many models to forecast because most models expect the [variance](https://mftokic.github.io/posts/2024-09-25-ts-fundamentals-glossary/#variance) of the data to stay the same over time (more on this in the stationary post), and a expanding time series violates this assumption. Consider one of our example time series in our dataset below. See how it grows not at a linear pace but a hockey stick like exponential pace? 
